# ai_planner_controller.py - Updated with MongoDB Integration
...
from mongodb import MongoDBController
...
class AIPlannerController:
    def __init__(self, mongodb_controller: MongoDBController):
        self.db = mongodb_controller
    
    async def generate_workout_plan(self, request: WorkoutPlanRequest, user_id: Optional[str] = None) -> WorkoutPlan:
        """Generate a workout plan using Google's Gemini API and save to MongoDB"""
        ...
        try:
            ...
            # Create workout plan dictionary for MongoDB
            workout_plan_data = {
                "user_id": user_id,
            ...
            # Save the workout plan to MongoDB
            plan_id = await self.db.save_workout_plan(workout_plan_data)
            
            # Retrieve the full document with ID for response
            workout_plan_data = await self.db.get_workout_plan(plan_id)
    ...
    async def get_workout_plan(self, plan_id: str) -> Optional[WorkoutPlan]:
        """Get a workout plan by ID from MongoDB"""
        try:
            plan_data = await self.db.get_workout_plan(plan_id)
            if not plan_data:
                return None
    ...
    async def get_user_workout_plans(self, user_id: str) -> List[WorkoutPlan]:
        """Get all workout plans for a user from MongoDB"""
        try:
            plans_data = await self.db.get_user_workout_plans(user_id)
            return [WorkoutPlan(**plan) for plan in plans_data]
        except Exception as e:
            raise Exception(f"Error retrieving user workout plans: {str(e)}")
    
    async def delete_workout_plan(self, plan_id: str) -> bool:
        """Delete a workout plan from MongoDB"""
        try:
            return await self.db.delete_workout_plan(plan_id)
        except Exception as e:
            raise Exception(f"Error deleting workout plan: {str(e)}")